...
float* get_adiacency_matrix_single(int n, int m, location *l, int *no, float *d){
	//Padding che rende le colonne della matrice multiple di 32
	/*Il padding comporta due benefici:
	 * 1) Elimina il ciclo resto
	 * 2) Elimina il ciclo quoziente nel loop unrolling
	 * All'inizio le colonne della matrice erano multiple di 8 perchè
	 * la loop vectorization prendeva 8 elementi per volta
	 * Ora diventano multiple di 32 dato che l'unrolling prende 32 elementi per volta
	 */
	int o = 0;
	int cols = n;
	if(n % 32 != 0){
	...
	*no = cols;
	 ...
	 */
	float* s = (float*)_mm_malloc(n*cols*sizeof(float), 32);
	int i, j;
	for(int k = 0; k < m; k++){
		i = l[k].x-1;
		j = l[k].y-1;
...
double* get_adiacency_matrix_double(int n, int m, location *l, int *no, double *d){
	...
	*no = cols;
	double* g = (double*)_mm_malloc(n*cols*sizeof(double),32);
	int i, j;
	for(int k = 0; k < m; k++){
		i = l[k].x-1;
		j = l[k].y-1;
