...
GRAPHD load_sparse_double(char* filename, int *n, int *m) {
	 ...
	 */
	GRAPHD g = (GRAPHD)_mm_malloc(nodes*nodes*sizeof(double),32); //alloca la struttura dati contenente il grafo
	/*
...
GRAPHS load_sparse_single(char* filename, int *n, int *m){
	 ...
	 */
	GRAPHS s = (GRAPHS)_mm_malloc(nodes*nodes*sizeof(float), 32); //alloca la struttura dati contenente il grafo
	/*
...
float* get_outdegree_single(int n, float *A){
	...
	float *d = (float *)_mm_malloc(n*sizeof(float), 32);
	for(int i = 0; i < n; i++){
		int out = 0;
		for(int j = 0; j < n; j++){
...
double* get_matrix_P_double(int n, double *A, double *d){
	double *P = (double *)_mm_malloc(n*n*sizeof(double), 32);
	for(int i = 0; i < n; i++){
		for(int j = 0; j < n; j++){
			if(A[i*n + j] != 0){
				P[i*n + j] = A[i*n + j]/d[i];
...
float* getVectorPiIn_single(int n, float *v){
	float* Pi=(float*)_mm_malloc(sizeof(float)*n,32);
	for (int i=0; i<n; i++){
		Pi[i]=v[i];
	}
	return Pi;
...
double* getVectorPiIn_double(int n, double *v){
	double* Pi=(double*)_mm_malloc(sizeof(double)*n,32);
	for (int i=0; i<n; i++){
		Pi[i]=v[i];
	}
	return Pi;
...
double* getPagerank_single(float *Pi0, float *P, double eps, int n){
	int stop = 0;
	float delta = 0;
	float *Pik = (float *)_mm_malloc(n*sizeof(float), 32);
	while(!stop){
	...
	double *Piconv = (double *) _mm_malloc(n*sizeof(double), 32);
	for(int i = 0; i < n; i++){
...
double* getPagerank_double(double *Pi0, double *P, double eps, int n){
	int stop = 0;
	double delta = 0;
	double *Pik = (double *)_mm_malloc(n*sizeof(double), 32);
	while(!stop){
